...
def map_config_to_env_vars(charm, **additional_env):
    ...
    """
    env_mapped_config = {k.replace("-", "_").upper(): v for k, v in charm.config.items()}
...
def get_machine_token(contract_token: str, contracts_url=DEFAULT_CONTRACTS_URL, proxies=None) -> str:
    ...
    headers = {
        'Authorization': 'Bearer {}'.format(contract_token),
        'Content-Type': 'application/json'
    }
    try:
        response = requests.post(
            url='{}/v1/context/machines/token'.format(contracts_url),
...
def get_resource_token(machine_token, contracts_url=DEFAULT_CONTRACTS_URL, proxies=None):
    ...
    try:
        req = requests.get(
            url='{}/v1/resources/{}/context/machines/livepatch-onprem'.format(
                contracts_url, RESOURCE_NAME),
            headers=headers)
        data = req.json()
